...
void APhysicsProjectile::Start()
{
	...
	if (bShowTrajectory) {
		for (FVector position : CurrentTrajectory) {
			DrawDebugSphere(GetWorld(), position, 5, 8, FColor::Green, true, 1, 0, 2);
		}
	}
...
{
	FVector CurrentMoveVector = CurrentTrajectory[TrajectoryCurrentIndex] - GetActorLocation(); //CurrentTrajectory = empty
	...
	if (FVector::Distance(newLocation, CurrentTrajectory[TrajectoryCurrentIndex]) <= MoveAccurency) {
		...
		if (TrajectoryCurrentIndex >= CurrentTrajectory.Num()) {
			if(VolumetricExplosion) Explode();
			Destroy();
		}else{
